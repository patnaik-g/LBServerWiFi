...
KasaPlug* KasaPlug::Find(const char* targetAlias, int timeoutMs, int maxRetries) {
    ...
    while(attempt++ < maxRetries) {
        ...
        if (sendto(sock, buf, Encrypt(CMD_INFO, strlen(CMD_INFO), 0, buf), 0, (struct sockaddr *)&addr, sizeof(addr)) > 0) {
...
int KasaPlug::SyncState() {
    // STATIC ALLOCATION:
    // Moves 1KB from Stack to BSS.
    // Critical for stability when called
...
bool KasaPlug::SetRelay(uint8_t target) {
    char buf[64];
    // Small enough to leave on stack
